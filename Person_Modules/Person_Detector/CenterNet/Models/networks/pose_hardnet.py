...
class HarDNetSeg(nn.Module):
    def __init__(self, num_layers, heads, pretrained, down_ratio, final_kernel,
        ...
        if num_layers == 85:
            ...
            self.last_proj = ConvLayer(518, 192, kernel=1)
            self.last_blk = HarDBlock(576, 64, 1.6, 8)
            self.skip_nodes = [1, 3, 5, 8]
            self.SC = [32, 32, 0]
            gr = [48, 32, 20]
            layers = [8, 8, 4]
            ch_list2 = [96 + self.SC[0], 64 + self.SC[1], 32 + self.SC[2]]
            channels = [48, 72, 96, 518]
            self.skip_lv = 2
            scales = [2 ** i for i in range(len(channels[self.first_level:]))]
