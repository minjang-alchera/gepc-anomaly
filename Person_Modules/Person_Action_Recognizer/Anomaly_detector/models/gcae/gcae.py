...
from Person_Modules.Person_Action_Recognizer.Anomaly_detector.models.graph.graph import Graph
from Person_Modules.Person_Action_Recognizer.Anomaly_detector.models.graph.st_graph_conv_block import ConvBlock
...
class GCAE(nn.Module):
    ...
    def encode(self, x):
        ...
        _, c, t, v = x.size()
        x = x.contiguous()
        x = x.view(M, c, t, v).permute(1, 2, 3, 0)
        x_ref = x
        x_size = x.size()
        #print("x.size()",x_size)
        x = x.contiguous()
    ...
    def build_dec(self, dec_ch_fac, dec_stride, dec_1st_residual=True):
        ...
        for i in range(1, len(dec_ch_fac)):
            if dec_stride[i] != 1:
                st_gcn_dec.append(nn.Upsample(scale_factor=(int(dec_stride[i]), 1), mode='bilinear'))
